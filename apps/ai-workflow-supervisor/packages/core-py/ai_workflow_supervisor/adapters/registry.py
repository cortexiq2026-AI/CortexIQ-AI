"""Wires SupervisorSettings to concrete provider instances.

To add a new provider: implement LLMProvider or SearchProvider in the
relevant llm/ or search/ subfolder, then add one branch here.
"""
...
def build_llm_provider(settings: SupervisorSettings) -> LLMProvider:
    ...
    raise ValueError(
        f"Unknown SUPERVISOR_LLM_PROVIDER '{settings.llm_provider}'. "
        "Expected one of: anthropic, openai, ollama."
    )


def build_search_provider(settings: SupervisorSettings) -> SearchProvider:
    ...
    raise ValueError(
        f"Unknown SUPERVISOR_SEARCH_PROVIDER '{settings.search_provider}'. "
